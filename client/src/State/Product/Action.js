...
export const findProducts=(reqData)=async(dispatch)=>{
    dispatch({type:FIND_PRODUCTS_REQUEST}) 
        ...
        const {minPrice,maxPrice,minDiscount,category,stock,pageNumber,pageSize,sort}=reqData;
        try{
            const {data}=api.get(`/api/products/minPrice=${minPrice}&maxPrice=${maxPrice}
...
export const findProductsById=(reqData)=async(dispatch)=>{
    ...
    const {productId}=reqData;
        try{
            const {data}=api.get(`/apu/products/id/${productId}`)
